...
class EmbeddrFindSimilarArtifactsNode(io.ComfyNode):
    ...
    def execute(cls, image, limit):
        ...
        # Endpoint in Plugin
        api_url = f"{base_url}/api/v2/plugins/embeddr-comfyui/find_similar"
        ...
        try:
            ...
            for item in items:
                art_id = item.get("id")
                content_url = f"{base_url}/api/v2/plugins/embeddr-comfyui/content/{art_id}"

                try:
                    img_resp = requests.get(content_url)
