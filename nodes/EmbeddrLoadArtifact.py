...
class EmbeddrLoadArtifactNode(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            category="Embeddr",
            inputs=[
                io.String.Input("artifact_id", default="",
                                tooltip="UUID of the artifact to load"),
                io.Boolean.Input("use_cache", default=True)
            ],
            outputs=[
                io.Image.Output("image"),
                io.Mask.Output("mask"),
                io.String.Output("artifact_id_out"),
            ],
        )

    @classmethod
    def execute(cls, artifact_id, use_cache):
        if not artifact_id:
        ...
        if use_cache and artifact_id in cls._cache:
            image, mask = cls._cache[artifact_id]
            return io.NodeOutput(image, mask, artifact_id)
        ...
        try:
            ...
            response = requests.get(endpoint, headers=content_headers)
            ...
            if use_cache:
                cls._cache[artifact_id] = (image, mask)

            return io.NodeOutput(image, mask, artifact_id, ui=ui.PreviewImage(image))

        except Exception as e:
            print(f"[Embeddr] Error loading artifact {artifact_id}: {e}")
